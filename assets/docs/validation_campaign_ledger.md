...
| `S28` | Successful checkpoint evaluation | `S24`, `S27` | `UNTESTED` | Rechecked 2026-09-24: canonical and prior isolated encoder/checkpoint paths remain absent (canonical checkpoint directory contains only `.gitkeep`), and the exact pinned BEiT snapshot is absent from its expected path and Hugging Face cache. Evaluation was not run. Restore the encoder, recreate S24, then evaluate its compatible checkpoint and retrieve the saved report. [Current gate recheck](../QA/validation_campaign/s33-s52-20260924/gate-recheck.json); [S31 prerequisite recheck](../QA/validation_campaign/s31/prerequisite-recheck-20260924.json); [2026-09-24 prerequisite probe](../QA/validation_campaign/s28/prerequisite-check-20260924.json). |
...
| `S35` | CXRMate-ED sensitivity canary | `S31` | `FAIL` / degraded | Rechecked 2026-09-24: live catalogue status remains `degraded`; the historical three-case run completed but produced only two distinct reports. The exact approved fixture directory is absent, so no substitutes were run and promotion remains unjustified. [Current gate recheck](../QA/validation_campaign/s33-s52-20260924/gate-recheck.json); [historical canary](../QA/inference_validation_runs/cxrmate-ed-sensitivity-20260919T154916Z.json). |
...
| `S37` | MedGemma gated-provider behavior | `S30` | `BLOCKED` | Rechecked 2026-09-24: the live catalogue reports `access_policy=gated`; no nonempty `HF_TOKEN` was present in the validation process or `settings/.env`. The stored credential store was not probed, and no install or generation was attempted. Continue blocked until authorized gated access and a supported credential are available. [Current gate recheck](../QA/validation_campaign/s33-s52-20260924/gate-recheck.json); [live API response](../QA/validation_campaign/s30-s32-20260924/catalog-api.json). |
...
| `S52` | Accessibility and responsive workflow audit | Stable Tiers 2–3 UI | `PARTIAL` | Focused inference-page audit on 2026-09-24: no horizontal overflow at 320x720, 768x1024, 1024x720, or 1440x900; named controls were keyboard reachable with a visible focus ring. Found and fixed the Help & Tips focus trap; a regression test verifies its two boundary anchors, and the rebuilt browser retest cycled focus inside the 15-control dialog, then Escape closed it and restored focus. Reduced-motion CSS exists, but runtime preference emulation and the broader Tier 2–3 route/modal matrix remain untested. [S33/S52 summary](../QA/validation_campaign/s33-s52-20260924/summary.md); [UI observations](../QA/validation_campaign/s33-s52-20260924/ui-observations.json); [modal regression test](../../app/client/src/app/components/modal-focus.directive.spec.ts). |
