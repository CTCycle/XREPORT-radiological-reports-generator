...
- S30–S32 were rechecked on 2026-09-24 from the current `develop` checkout. S30 and S32 pass after 55 focused inference unit tests, six live API E2E tests, Ruff, and saved rendered-browser screenshots. The campaign also fixed unbounded endpoint-side upload copying and isolated a catalog-test monkeypatch leak. S31 remains partial because no fresh public-model install/reuse/repair/delete cycle or real inference-based readiness promotion was run. S35 remains failed/degraded because its exact fixture directory is absent; S37 remains blocked because gated access is not configured. See the [S30–S32 summary](../QA/validation_campaign/s30-s32-20260924/summary.md) and its API, unit, browser, runtime, and prerequisite evidence.
...
| `S31` | Lightweight public-model installation lifecycle | `S30` | `PARTIAL` | Rechecked 2026-09-24: manifest, installation-manager, provider, adapter, and persistence-guard unit slice passed (`55 passed`), including local snapshot integrity/lifecycle behavior. No fresh remote install, no-redownload reuse, real-model repair/delete, or inference-receipt readiness promotion was exercised; the isolated API catalogue remained clean. [Campaign summary](../QA/validation_campaign/s30-s32-20260924/summary.md); [unit log](../QA/validation_campaign/s30-s32-20260924/unit-tests-final.log); [live API response](../QA/validation_campaign/s30-s32-20260924/catalog-api.json). |
...
| `S35` | CXRMate-ED sensitivity canary | `S31` | `FAIL` / degraded | Rechecked 2026-09-24: the live catalogue still reports `degraded`; the historical three-case run completed but produced only two distinct reports. The exact approved fixture directory is absent, so no substitute cases were run and promotion remains unjustified. [Campaign summary](../QA/validation_campaign/s30-s32-20260924/summary.md); [gate recheck](../QA/validation_campaign/s30-s32-20260924/gate-recheck.json); [historical canary](../QA/inference_validation_runs/cxrmate-ed-sensitivity-20260919T154916Z.json). |
...
| `S37` | MedGemma gated-provider behavior | `S30` | `BLOCKED` | Rechecked 2026-09-24: the live catalogue reports `access_policy=gated`, and no `HF_TOKEN` was present in the validation process or configured in `settings/.env`. No install or generation was attempted. Continue blocked until authorized access is configured. [Campaign summary](../QA/validation_campaign/s30-s32-20260924/summary.md); [gate recheck](../QA/validation_campaign/s30-s32-20260924/gate-recheck.json); [live API response](../QA/validation_campaign/s30-s32-20260924/catalog-api.json). |
