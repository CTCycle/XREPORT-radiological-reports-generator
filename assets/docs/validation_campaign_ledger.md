...
- S23–S26 were exercised on 2026-09-23 from a clean `develop` baseline at `3846609cc68646b501cd480c2a11938fa2278f8b`, using an isolated SQLite resource root, eight synthetic rows, the pinned DistilBERT tokenizer snapshot, the pinned BEiT encoder, and an RTX 3060. The exact validated changes were committed as `d665ee078ef5a878113f3db03c518834b5afdd68`; hosted CI [35904882475](https://github.com/CTCycle/XREPORT-radiological-reports-generator/actions/runs/35904882475) passed every configured backend and client check. S23 is `PARTIAL`: rendered source-row path containment and viewer navigation passed, and deleting both disposable datasets left no orphans, but deleting the source cascades its records and leaves the surviving processed dataset empty and not trainable. S24 passed a real one-epoch CUDA training job; S25 passed cancellation after progress, worker exit, and resume to two persisted epochs; S26 passed ready listing/metadata, referenced/unsafe deletion guards, and safe artifact plus registry removal. The campaign exposed and fixed process-level `XREPORT_RESOURCES_DIR` loss during dotenv loading, a fixture-history root cause for `ISSUE-003`, and the source-path table overflow reported by the user. The focused dataset/training E2E suite passed (`8 passed`), the environment regression passed (`3 passed`), Ruff and frontend build passed. See the [S23–S26 summary](../QA/validation_campaign/s23-s26/summary-20260923.md); synthetic training is technical-path evidence only, not a clinical or performance claim.
...
| `S23` | Dataset viewer and deletion | `S22` | `PARTIAL` | Rendered long-path row containment and eight-image viewer navigation passed. Source deletion cleared the processed dataset's source reference but cascaded source records and all training samples; processed dataset remained listed with zero rows. Processed dataset deletion removed all dependent records/runs with SQLite integrity and FK checks clean. See [summary](../QA/validation_campaign/s23-s26/summary-20260923.md), [UI observations](../QA/validation_campaign/s23-s26/s23-ui-observations.json), and [deletion receipt](../QA/validation_campaign/s23-s26/s23-deletion.json). Resolve the processed-data ownership/deletion contract before promoting. |
...
| `S28` | Successful checkpoint evaluation | `S24`, `S27` | `UNTESTED` | Not run: isolated runtime had no S24 checkpoint and no local pinned BEiT encoder snapshot to recreate one offline. Restore the pinned encoder, rerun S24, then evaluate its compatible checkpoint and retrieve the saved report. [Prerequisite probe](../QA/validation_campaign/s27/s28-prerequisite-check.json). |
