...
- Current application source and validation/test revision: `37a4b78e6ca939f8a2b6cb9e29c8fa7d46a3d41e` (`develop`). The latest hosted validation revision is `5cbe2a7c86ec828ff8d9cecd35a55b94f9e9d0e3`, a docs-only descendant. Hosted CI run `35860925179` failed the client unit stage on that revision; its same-SHA failed-job rerun reproduced two desktop-dialog assertions. Backend, PostgreSQL, API, client build/server, and lint stages passed. Earlier same-code revision `16794b4` passed all stages on rerun `35859367232`. S20's focused API/service suite passed locally (`12 passed`); see the [S20 summary](../QA/validation_campaign/s20/summary-20260923.md). S02 remains covered by the [2026-09-23 Tier 0 summary](../QA/validation_campaign/tier-0/summary-20260923.md).
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `S00` | Restore current CI baseline | yes | yes | `PARTIAL` | Current docs/evidence revision `5cbe2a7` has hosted CI run `35860925179`; the client unit stage failed both on the push attempt and its same-SHA rerun due to two desktop-dialog assertions. Backend, database, API, build, frontend-server, and lint stages passed. Earlier same-code revision `16794b4` passed all stages on run `35859367232` after rerun, but the current baseline remains partial until the repeatable client gate is resolved. | [2026-09-22 Tier 0 summary](../QA/validation_campaign/tier-0/summary-20260922.md); [failed current CI run 35860925179](https://github.com/CTCycle/XREPORT-radiological-reports-generator/actions/runs/35860925179); [prior green run 35859367232](https://github.com/CTCycle/XREPORT-radiological-reports-generator/actions/runs/35859367232) | Resolve the repeatable `desktop-dialog.service.spec.ts` failures, then rerun all hosted gates. |
...
| `S03` | Static quality, API contract, and client build gates | yes | yes | `PARTIAL` | On current docs/evidence revision `5cbe2a7`, hosted CI passed Ruff, Pyright, the backend suite, PostgreSQL contract, API E2E, client build/server, and lint; the client unit stage repeatedly failed two desktop-dialog assertions. The same-code revision `16794b4` had a green rerun, but the latest baseline is not stable. Current-scope local Windows production build and rendered checks remain in the dated execution summary. | [2026-09-22 execution summary](../QA/validation_campaign/tier-0/summary-20260922.md); [current CI run 35860925179](https://github.com/CTCycle/XREPORT-radiological-reports-generator/actions/runs/35860925179) | Resolve the repeatable desktop-dialog test failures and rerun hosted client unit tests. |
