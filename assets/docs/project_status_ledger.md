...
| `persistence.sqlite_migrations` | `VALIDATED` | SQLite startup/initialization reaches the checked-in Alembic head and persists application settings without implicit schema stamping. | [Tier 0 execution summary](../QA/validation_campaign/tier-0/summary-20260921.md) records fresh/restart head `f48a7c2e91b6` and fail-closed unversioned-schema startup; [backend smoke](../QA/xreport-backend-smoke-20260919.log); [database initialization tests](../../app/tests/unit/test_database_initialization.py); [settings migration validation](../QA/settings-migration-validation-20260917.md) | Existing unversioned or incompatible schemas intentionally fail closed. | — | 2026-09-21 | unit + integration + manual | [persistence](architecture/persistence.md); [architecture review](architecture/architecture_review.md) | Recheck migration upgrade and rollback safety for every new revision. |
