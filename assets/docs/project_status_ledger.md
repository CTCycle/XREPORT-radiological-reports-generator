...
- S23–S26 have now been exercised in an isolated synthetic campaign. Training,
  ...
  checkpoint-evaluation and custom-inference workflows. Packaged desktop
  workflows remain validation debt.
...
| `runtime.desktop.packaged` | `UNVALIDATED` | Tauri CPU/CUDA runtime extraction, portable/MSI packaging, startup, shutdown, and user-data-root isolation. | [desktop packaging tests](../../app/tests/unit/test_desktop_packaging.py); [2026-09-17 E2E note](../QA/e2e-validation-20260917.md) records `cargo check` passed; [2026-09-25 S40 dev-shell evidence](../QA/validation_campaign/s40-s42-desktop-20260925/S40-manual-dev-shell.md) records diagnostic shell startup only. | No current CPU/CUDA portable/MSI smoke report exists. S40 did not prove WebView rendering or packaged data-root isolation. | — | 2026-09-25 | unit + build-check + manual diagnostic | [deployment](runtime/deployment.md); [runtime modes](runtime/modes.md) | Build both variants and run the documented packaged smoke checks, preserving reports under `assets/QA/desktop/`. |
