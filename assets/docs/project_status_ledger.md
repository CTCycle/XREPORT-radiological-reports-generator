...
| `workflow.checkpoint_evaluation` | `UNVALIDATED` | Compatible checkpoint evaluation, associated dataset resolution, metrics, and report persistence. | [evaluation tests](../../app/tests/unit/test_evaluation.py); [validation job tests](../../app/tests/unit/test_validation_job_semantics.py); [2026-09-24 S28 prerequisite probe](../QA/validation_campaign/s28/prerequisite-check-20260924.json); [fresh S28 prerequisite recheck](../QA/validation_campaign/s31/prerequisite-recheck-20260924.json) | Preflight and failure semantics are tested; successful live evaluation is not evidenced. The checked campaign and canonical checkpoint directories contain no S24 artifact, and the exact pinned BEiT snapshot is absent from the expected path and Hugging Face cache. | Compatible S24 checkpoint and pinned encoder unavailable locally | 2026-09-24 | unit + prerequisite inspection | [workflows](operations/workflows.md); [backend API](architecture/backend_api.md) | Restore the pinned encoder, recreate S24, then run S28 and retrieve its persisted evaluation report. |
| `workflow.inference.generation_pipeline` | `WORKING` | One exact pinned public-model installation/readiness lifecycle with real study API generation, provenance, job completion, restart reuse, report persistence, and deletion. | [S31 live lifecycle summary](../QA/validation_campaign/s31/summary-20260924.md); [real generation receipt](../QA/validation_campaign/s31/real-inference-job.json); [restart/reuse evidence](../QA/validation_campaign/s31/restart-reuse.json); [provider tests](../../app/tests/unit/test_huggingface_provider.py); [S30–S32 contract campaign](../QA/validation_campaign/s30-s32-20260924/summary.md) | Technical proof is limited to one open model and one public image; it establishes neither model quality nor clinical performance. Rendered Generate/Cancel/edit/export/provenance/persistence flow remains S33, while timeout and persistence-failure behavior and other providers remain untested. | — | 2026-09-24 | live API + manual restart/runtime | [local inference models](runtime/local_inference_models.md); [workflows](operations/workflows.md) | Cover the rendered S33 Generate/review flow, then validate timeout/persistence failure and multi-case quality for supported models. |
