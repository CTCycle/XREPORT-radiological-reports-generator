...
- S23–S26 have now been exercised in an isolated synthetic campaign. Training,
  ...
  technical scope; source deletion still empties the surviving processed
  dataset, so S23 remains partial. S27 successful dataset validation is now
  passed for its small synthetic scope. Successful checkpoint evaluation
  (S28) and packaged desktop workflows remain validation debt.
...
| Component | Status | Scope | Evidence | Known Issues | Blocker | Last Validated | Validation Level | Related Docs | Next Action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `workflow.dataset_upload_and_preparation` | `VALIDATED` | Synthetic non-empty upload through image matching, partial confirmation, processing, persistence, and downstream-ready processed dataset. | [S21 summary](../QA/validation_campaign/s21/summary-20260923.md); [S22 summary](../QA/validation_campaign/s22/summary-20260923.md); [S23–S26 summary](../QA/validation_campaign/s23-s26/summary-20260923.md); [image scanning tests](../../app/tests/unit/test_preparation_image_scanning.py); [dataset API/UI tests](../../app/tests/e2e/test_dataset_workflow_api.py); [dataset page test](../../app/tests/e2e/test_dataset_workflow_ui.py) | Evidence uses an eight-row synthetic corpus; large/real-world data, packaged desktop operation, and clinical quality remain outside this status. S23 found that deleting the source preserves processed identity and clears its source reference but cascades its dataset records and training samples, leaving it empty and not trainable; see `ISSUE-005`. | — | 2026-09-23 | unit + API/UI E2E + live processing + restart persistence + deletion observation | [workflows](operations/workflows.md); [backend API](architecture/backend_api.md) | Define and implement the source/processed record ownership and deletion contract, then rerun S23 before upgrading its gate. |
...
| `ui.dataset_and_training_surfaces` | `PARTIAL` | Dataset and Training route rendering; dataset table source-path containment and title; rendered eight-image viewer navigation; processed dataset metadata and training surface inspection; S27 successful validation summary review. | [S23–S26 summary](../QA/validation_campaign/s23-s26/summary-20260923.md); [S27 summary](../QA/validation_campaign/s27/summary-20260923.md); [row/viewer captures](../QA/validation_campaign/s23-s26/s23-ui-observations.json); [dataset workflow E2E](../../app/tests/e2e/test_dataset_workflow_ui.py); [S10 route matrix](../QA/validation_campaign/s10/summary-20260922.md) | User-reported long Windows source path overflow was reproduced and fixed with zero-min-width ellipsis containment and full path in the title. Checkpoint evaluation UI and complete dataset lifecycle UI states remain outside this scope; S23 deletion lifecycle is partial due to `ISSUE-005`. | — | 2026-09-23 | E2E + manual | [UI patterns](ui/components_and_patterns.md); [workflows](operations/workflows.md) | Revalidate the Dataset table at narrow/wide widths when its layout changes; finish S23 deletion semantics and S28 evaluation view. |
...
| `ISSUE-005` | `workflow.dataset_upload_and_preparation` | `MEDIUM` | Deleting a source dataset preserves its processed dataset and clears `ProcessingRun.source_dataset_id`, but cascades source records and their linked training samples. | Processed dataset remains listed with zero rows and can no longer be trained after its source is deleted. | [S23 deletion receipt](../QA/validation_campaign/s23-s26/s23-deletion.json) records eight training samples before deletion, zero afterward, preserved processing history, no orphans, and SQLite integrity `ok`. | `TrainingSample.record_id` references `DatasetRecord` with `ON DELETE CASCADE`; current ownership keeps processed samples dependent on source records. | — | `OPEN` — S23 remains partial until the processed-data ownership/deletion contract is addressed. | Define expected delete semantics and make processed training samples independent of source-record cascades; rerun dataset deletion, trainability, and integrity checks. | [persistence](architecture/persistence.md); [workflows](operations/workflows.md) |
