...
class DatasetEvents:
    ...
    def get_description_from_image(self, image_name: str) -> Any | str:
        ...
        mask = (
            ...
            .astype(str)
            .str.contains(image_no_ext, case=False, na=False)
        )
    ...
    def run_dataset_builder(
        ...
        logger.info(
            f"Tokenizing text corpus using pretrained {tokenization.tokenizer_id} tokenizer"
        )
...
class ValidationEvents:
    ...
    def run_model_evaluation_pipeline(
        ...
        metric_map = {
            ...
            "BLEU_score": scoring.calculate_BLEU_score,
        }
...
class ModelEvents:
    ...
    def run_inference_pipeline(
        ...
        logger.info(
            f"Start generating reports using model {os.path.basename(checkpoint_path)}"
        )
        logger.info(
            f"{len(img_paths)} images have been found and are ready for inference pipeline"
        )
