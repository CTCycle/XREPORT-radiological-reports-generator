...
def digest_entries(entries: list[tuple[Path, str]]) -> tuple[str, int, list[dict[str, object]]]:
...
def zip_entry(zf: zipfile.ZipFile, source: Path, name: str) -> None:
    ...
    with source.open("rb") as stream, zf.open(info, "w", force_zip64=True) as destination:
        while chunk := stream.read(CHUNK_SIZE):
...
def main() -> int:
    ...
    with zipfile.ZipFile(
        ...
        manifest_info = zipfile.ZipInfo("runtime-manifest.json", date_time=(1980, 1, 1, 0, 0, 0))
        manifest_info.compress_type = zipfile.ZIP_DEFLATED
        manifest_info.create_system = 0
        manifest_info.external_attr = (stat.S_IFREG | 0o644) << 16
        archive.writestr(manifest_info, json.dumps(manifest, indent=2, sort_keys=True).encode("utf-8"))
    temporary.replace(args.output)
    args.audit.parent.mkdir(parents=True, exist_ok=True)
    args.audit.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
