...
set "VENV_PYTHON=%APP_DIR%\\server\\.venv\\Scripts\\python.exe"
...
set "DOTENV=%PROJECT_ROOT%\\settings\\.env"
...
set "APP_TEST_FRONTEND_URL=%APP_TEST_BACKEND_URL%"

REM Check for Python (require uv-created app\\server\\.venv)
if exist "%VENV_PYTHON%" (
    set "PYTHON_CMD=%VENV_PYTHON%"
) else (
    echo [ERROR] app\\server\\.venv not found at "%VENV_PYTHON%".
    echo [ERROR] Run start_on_windows.bat to create the environment.
    exit /b 1
)
...
REM Check for pytest/playwright in the existing app\\server\\.venv only (no installs here)
if /i "%OPTIONAL_DEPENDENCIES%"=="true" (
    "%PYTHON_CMD%" -c "import pytest" >nul 2>&1
    if %ERRORLEVEL% neq 0 (
        echo [ERROR] pytest not installed in app\\server\\.venv.
        echo [ERROR] Set OPTIONAL_DEPENDENCIES=true and run start_on_windows.bat.
        exit /b 1
    )

    "%PYTHON_CMD%" -c "import playwright" >nul 2>&1
    if %ERRORLEVEL% neq 0 (
        echo [ERROR] playwright not installed in app\\server\\.venv.
        echo [ERROR] Set OPTIONAL_DEPENDENCIES=true and run start_on_windows.bat.
        exit /b 1
    )
    ...
    if %ERRORLEVEL% neq 0 (
        echo [ERROR] psutil not installed in app\\server\\.venv.
        echo [ERROR] Set OPTIONAL_DEPENDENCIES=true and run start_on_windows.bat.
        exit /b 1
    )
)
...
if "%BACKEND_RUNNING%"=="0" (
    ...
    start "" /B /D "%APP_DIR%\\server" "%PYTHON_CMD%" -m uvicorn server.app:app --host %FASTAPI_HOST% --port %FASTAPI_PORT%
...
cd /d "%PROJECT_ROOT%"
"%PYTHON_CMD%" -m pytest "%APP_DIR%\\tests" -v --tb=short %*
