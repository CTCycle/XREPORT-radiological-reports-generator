...
set "XREPORT_CACHE_ROOT=%RUNTIME_CACHE_DIR%"
set "XDG_CACHE_HOME=%RUNTIME_CACHE_DIR%"
set "PYTEST_CACHE_DIR=%RUNTIME_CACHE_DIR%\pytest"
set "PYTEST_BASETEMP=%RUNTIME_CACHE_DIR%\pytest-tmp\full-%RANDOM%"
...
for %%D in ("%RUNTIME_CACHE_DIR%" "%UV_CACHE_DIR%" "%PIP_CACHE_DIR%" "%NPM_CACHE_DIR%" "%PLAYWRIGHT_BROWSERS_PATH%" "%PYTEST_CACHE_DIR%" "%PYTEST_BASETEMP%" "%RUFF_CACHE_DIR%" "%MYPY_CACHE_DIR%" "%PYTHONPYCACHEPREFIX%" "%RUNTIME_CACHE_DIR%\coverage") do if not exist "%%~D" mkdir "%%~D" >nul 2>&1
...
if /i "%STANDARD_TEST_SKIP_LIVE_SERVERS%"=="false" if "%HAS_E2E%"=="1" (
  ...
  if errorlevel 1 (
    ...
    start "" /B /D "%BACKEND_WORKDIR%" "%PYTHON_CMD%" -m uvicorn %UVICORN_APP% --host %FASTAPI_HOST% --port %FASTAPI_PORT% --log-level warning >nul 2>&1
  ...
  if /i "%STANDARD_TEST_SKIP_FRONTEND%"=="false" if exist "%CLIENT_DIR%\package.json" (
    ...
    if errorlevel 1 (
      ...
      start "" /B /D "%CLIENT_DIR%" "%NPM_CMD%" run preview -- --host %UI_HOST% --port %UI_PORT% >nul 2>&1
