...
def test_runtime_bundle_rejects_mutable_or_log_artifacts(tmp_path: Path) -> None:
    ...
    with (
        zipfile.ZipFile(output) as source,
        zipfile.ZipFile(missing_required, "w") as target,
    ):
    ...
    portable.write_bytes(
        b"MZ" + archive_bytes + b"XRPZIP01" + struct.pack("<QQ", 2, len(archive_bytes))
    )
    assert (
        verify_portable(
            portable,
    ...
    invalid_overlay.write_bytes(
        b"MZ"
        + archive_bytes
        + b"XRPZIP01"
        + struct.pack("<QQ", 1, len(archive_bytes) + 3)
    )
    ...
    with (
        zipfile.ZipFile(output) as source,
        zipfile.ZipFile(missing_backend, "w") as target,
    ):
        for info in source.infolist():
...
def test_desktop_build_inputs_are_locked_and_placeholder_free() -> None:
    requirements = (
        Path(__file__).parents[2] / "desktop" / "build" / "cpu-runtime-requirements.txt"
...
def test_packaged_desktop_processes_are_windowless() -> None:
    ...
    backend = (desktop_root / "src-tauri" / "src" / "backend.rs").read_text(
        encoding="utf-8"
    )
