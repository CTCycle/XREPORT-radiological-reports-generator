...
def manager_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    monkeypatch.setattr(installation_module, "HF_HUB_CACHE_DIR", resources / "models" / "huggingface" / "hub-cache")
...
def test_stage_downloads_only_approved_files_and_writes_verified_metadata(
    ...
    def fake_download(**kwargs: object) -> str:
        ...
        (target / "model.safetensors").write_text("ok", encoding="utf-8")
        return str(target)

    monkeypatch.setattr(installation_module, "snapshot_download", fake_download)
