...
fn spawn_backend(app_handle: &tauri::AppHandle, state: &BackendChildState) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        let workspace_root = find_workspace_root(app_handle).ok_or_else(|| {
            String::from(
                "Cannot resolve packaged backend workspace (missing app/server/pyproject.toml).",
            )
        })?;
        ...
        let venv_dir = if runtime_root == workspace_root {
            server_dir.join(".venv")
        } else {
            runtime_root.join("server").join(".venv")
        };
        ...
        let child = child_command
            .current_dir(&server_dir)
            .env("PYTHONPATH", app_dir.to_string_lossy().to_string())
