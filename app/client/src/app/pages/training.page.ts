...
interface StoredEvaluationJob { jobId: string; metrics: string[]; status?: 'pending' | 'running' | 'completed' | 'failed' | 'cancelled'; progress?: number; }
@Component({
  standalone: true,
  selector: 'app-training-page',
  imports: [CommonModule, ReactiveFormsModule, NgIcon, CheckpointEvaluationReportModalComponent, ModalFocusDirective, NewTrainingWizardComponent, TrainingDashboardComponent],
  ...
  template: `
    ...
    @if (evaluationOpen()) { <div class="modal-backdrop" role="presentation"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="evaluation-title" appModalFocus (modalEscape)="evaluationOpen.set(false)"><div class="modal-header"><h2 id="evaluation-title">Checkpoint Evaluation</h2><button type="button" class="btn-icon-small" aria-label="Close" (click)="evaluationOpen.set(false)"><ng-icon name="lucideX"/></button></div><p>Evaluate <strong>{{ evaluationCheckpoint()?.name }}</strong> against the processed validation set.</p><div [formGroup]="evaluationForm"><label class="checkbox-label"><input type="checkbox" formControlName="evaluationReport"/> Evaluation report</label><label class="checkbox-label"><input type="checkbox" formControlName="bleuScore"/> BLEU score</label></div>@if (evaluationStatus()) { <div class="upload-status" [class.success]="evaluationStatus() === 'completed'" [class.error]="evaluationStatus() === 'failed'">{{ evaluationStatus() }}</div> }<div class="modal-footer"><button type="button" class="btn btn-secondary" (click)="evaluationOpen.set(false)">Close</button><button type="button" class="btn btn-primary" (click)="runEvaluation()" [disabled]="evaluationBusy()">{{ evaluationBusy() ? 'Evaluating…' : 'Run evaluation' }}</button></div></section></div> }
    @if (metadataModal()) { <div class="modal-backdrop" role="presentation"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="metadata-title" appModalFocus (modalEscape)="metadataModal.set(null)"><div class="modal-header"><h2 id="metadata-title">{{ metadataModal()?.title }}</h2><button type="button" class="btn-icon-small" aria-label="Close" (click)="metadataModal.set(null)"><ng-icon name="lucideX"/></button></div><p>{{ metadataModal()?.subtitle }}</p><pre>{{ metadataModal()?.body }}</pre></section></div> }
    <app-checkpoint-evaluation-report-modal [open]="evaluationReportOpen()" [checkpoint]="evaluationCheckpoint()?.name ?? null" [loading]="evaluationReportLoading()" [error]="evaluationReportError()" [progress]="evaluationReportProgress()" [report]="evaluationReport()" (closed)="evaluationReportOpen.set(false)"/>
  `,
...
export class TrainingPage {
  ...
  readonly evaluationBusy = signal(false);
  readonly evaluationStatus = signal<string | null>(null);
  ...
  readonly evaluationForm = inject(NonNullableFormBuilder).group({ evaluationReport: true, bleuScore: false });
  ...
  async loadDatasets() { const result = await this.api.getProcessedDatasetNames(); if (result.result) this.datasets.set(result.result.datasets); }
  async loadCheckpoints() { const result = await this.api.getCheckpoints(); if (result.result) this.checkpoints.set(result.result.checkpoints); }
  selectDataset(dataset: DatasetInfo) { this.selectedDataset.set(dataset); }
  async showDatasetMetadata(dataset: DatasetInfo) { const result = await this.api.getProcessingMetadata(dataset.name); this.metadataModal.set({ title: 'Dataset Metadata', subtitle: dataset.name, body: result.result ? JSON.stringify(result.result.metadata, null, 2) : result.error ?? 'No metadata found' }); }
  async deleteDataset(dataset: DatasetInfo) { if (!confirm(`Delete dataset "${dataset.name}"? This cannot be undone.`)) return; const result = await this.api.deleteDataset(dataset.name); if (!result.result) this.trainingError.set(result.error); await this.loadDatasets(); }
  ...
  async deleteCheckpoint(checkpoint: CheckpointInfo) { if (!confirm(`Delete checkpoint "${checkpoint.name}"? This cannot be undone.`)) return; const result = await this.api.deleteCheckpoint(checkpoint.name); if (!result.result) this.trainingError.set(result.error); await this.loadCheckpoints(); }
  ...
  async startTraining() { if (!this.selectedDataset()) return; if (this.trainingForm.invalid) { this.trainingForm.markAllAsTouched(); this.trainingError.set('Enter positive values for all training parameters.'); return; } this.isLoading.set(true); this.trainingError.set(null); const started = await this.api.startTraining(this.startConfig()); if (!started.result) { this.isLoading.set(false); this.trainingError.set(started.error ?? 'Training failed'); return; } this.newWizardOpen.set(false); this.isLoading.set(false); this.pollTraining(started.result.job_id); }
  async resumeTraining() { if (!this.selectedCheckpoint()) return; if (this.resumeForm.invalid) { this.resumeForm.markAllAsTouched(); this.trainingError.set('Additional epochs must be at least 1.'); return; } this.isLoading.set(true); const started = await this.api.resumeTraining(this.selectedCheckpoint(), this.resumeForm.getRawValue().additionalEpochs); if (!started.result) { this.isLoading.set(false); this.trainingError.set(started.error ?? 'Resume training failed'); return; } this.resumeWizardOpen.set(false); this.isLoading.set(false); this.pollTraining(started.result.job_id); }
  private pollTraining(jobId: string) { this.appState.updateDashboard({ isTraining: true, currentEpoch: 0, progressPercent: 0, chartData: [], availableMetrics: [], epochBoundaries: [], logEntries: [`Training job started (${jobId}).`] }); this.polling.poll((id) => this.api.getTrainingJobStatus(id), jobId, 2).pipe(takeUntilDestroyed(this.destroyRef)).subscribe((status) => { const result = asRecord(status.result); if (result) { const chartData = this.parseChartData(result['chart_data']); this.appState.updateDashboard({ isTraining: !['completed', 'failed', 'cancelled'].includes(status.status), currentEpoch: readNumber(result['current_epoch']), totalEpochs: readNumber(result['total_epochs']), loss: readNumber(result['loss']), valLoss: readNumber(result['val_loss']), accuracy: readNumber(result['accuracy']), valAccuracy: readNumber(result['val_accuracy']), progressPercent: readNumber(result['progress_percent']), elapsedSeconds: readNumber(result['elapsed_seconds']), chartData: chartData ?? undefined, availableMetrics: readStringArray(result['available_metrics']) ?? undefined, epochBoundaries: readNumberArray(result['epoch_boundaries']) ?? undefined }); } if (['completed', 'failed', 'cancelled'].includes(status.status)) { this.appState.updateDashboard({ isTraining: false }); if (status.status !== 'completed') this.trainingError.set(status.error ?? `Training ${status.status}`); void this.loadCheckpoints(); } }); }
  ...
  async stopTraining() { const result = await this.api.getTrainingStatus(); if (result.result?.job_id) await this.api.cancelTrainingJob(result.result.job_id); this.appState.updateDashboard({ isTraining: false }); }
  openEvaluation(checkpoint: CheckpointInfo) { if (!checkpoint) return; this.evaluationCheckpoint.set(checkpoint); this.evaluationStatus.set(null); this.evaluationReport.set(null); this.evaluationReportError.set(null); this.evaluationReportOpen.set(false); this.evaluationOpen.set(true); }
  async runEvaluation() { const checkpoint = this.evaluationCheckpoint(); if (!checkpoint) return; const evaluationMetrics = this.evaluationForm.getRawValue(); const metrics = [evaluationMetrics.evaluationReport ? 'evaluation_report' : '', evaluationMetrics.bleuScore ? 'bleu_score' : ''].filter(Boolean); if (!metrics.length) return; this.evaluationBusy.set(true); const started = await this.api.evaluateCheckpoint(checkpoint.name, metrics, 10, undefined, 42); if (!started.result) { this.evaluationBusy.set(false); this.evaluationStatus.set(started.error ?? 'Evaluation failed'); return; } this.evaluationJobs.update((jobs) => ({ ...jobs, [checkpoint.name]: { jobId: started.result!.job_id, metrics, status: 'pending' } })); this.storage.writeRecord(EVALUATION_STORAGE_KEY, this.evaluationJobs()); this.polling.poll((jobId) => this.api.getCheckpointEvaluationJobStatus(jobId), started.result.job_id, 2).pipe(takeUntilDestroyed(this.destroyRef)).subscribe(async (status) => { if (['completed', 'failed', 'cancelled'].includes(status.status)) { this.evaluationBusy.set(false); this.evaluationStatus.set(status.status); this.evaluationJobs.update((jobs) => ({ ...jobs, [checkpoint.name]: { ...(jobs[checkpoint.name]!), status: status.status, progress: status.progress } })); this.storage.writeRecord(EVALUATION_STORAGE_KEY, this.evaluationJobs()); if (status.status === 'completed') { this.evaluationOpen.set(false); this.evaluationReportOpen.set(true); this.evaluationReportLoading.set(true); this.evaluationReportProgress.set(100); const report = await this.api.getCheckpointEvaluationReport(checkpoint.name); this.evaluationReportLoading.set(false); if (report.result) this.evaluationReport.set(report.result); else this.evaluationReportError.set(report.error ?? 'Failed to load evaluation report'); } } }); }
}
