...
@Component({
  ...
  template: `
    <main class="dataset-container"><app-feature-tip guidanceId="dataset-first-use" [version]="1" title="Build a training-ready dataset" message="Choose a valid image folder and data file, Load Dataset, then select exactly one row before building." /><div class="dataset-guidance-helper"><span>Dataset processing</span><app-help-popover label="Explain dataset processing" title="Dataset processing" body="Load the source first, select exactly one stored dataset, then choose sampling, validation split, tokenizer, and report limits before Build Dataset." /></div><div class="layout-rows">
